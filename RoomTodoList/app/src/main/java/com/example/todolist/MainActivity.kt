...
class MainActivity : AppCompatActivity(), OnItemLongClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // suspend 함수인 getAllTodoList()를 호출하여 할 일 목록을 불러옴
        // CoroutineScope.launch를 통해 코루틴을 시작하여 비동기적으로 getAllTodoList()를 호출
    ...
    private suspend fun getAllTodoList() {
        // withContext를 사용하여 IO 스레드에서 todoDao.getAllTodo()를 호출하여 모든 Todo를 가져옴
        todoList = withContext(Dispatchers.IO) { ArrayList(todoDao.getAllTodo()) }
        setRecyclerView()
    }
    ...
    override fun onRestart() {
        ...
        super.onRestart()
        // getAllTodoList()를 호출하여 모든 할 일 목록을 다시 불러오는 작업을 수행
        CoroutineScope(Dispatchers.Main).launch {
            getAllTodoList()
        }
    }
    ...
    private fun deleteTodo(position: Int) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            // UI 스레드에서 어댑터에 변경 사항을 알리고, 사용자에게 "삭제되었습니다."라는 알림을 표시
